...
export default function MapScreen() {
    ...
    useEffect(() => {
        ...
        const fetchCurrentLocation = async (): Promise<void> => {
            try {
                ...
                if (permissionStatus !== 'granted') { // grantの文字列を直接書く方法を回避できないか？
                    if (!isMounted) {
                        return;
                    }
                    setLocationState({
                        region: null,
                        isLoading: false,
                        errorMsg: '位置情報の権限が拒否されました', // 任意の文字列も別の場所にまとめたい
                    });
                    return;
                }
    ...
    return (
        ...
        >
            <Marker coordinate={locationState.region} title="現在地" />
        </MapView>
    );
}
